    ...
    panel.columns.forEach(column => {
      if (column.metrics.some(filter)) {
        column.metrics.filter(filter).forEach(metric => {
          _.set(doc, `aggs.${column.id}.aggs.timecolumn.aggs.${metric.id}-numerator.filter`, {
            query_string: { query: metric.numerator || '*', analyze_wildcard: true }
          });
          _.set(doc, `aggs.${column.id}.aggs.timecolumn.aggs.${metric.id}-denominator.filter`, {
          ...
          if (metric.metric_agg !== 'count' && bucketTransform[metric.metric_agg]) {
            ...
            _.set(doc, `aggs.${column.id}.aggs.timecolumn.aggs.${metric.id}-numerator.aggs`, aggBody);
            _.set(doc, `aggs.${column.id}.aggs.timecolumn.aggs.${metric.id}-denominator.aggs`, aggBody);
            numeratorPath = `${metric.id}-numerator>metric`;
            denominatorPath =  `${metric.id}-denominator>metric`;
          }

          _.set(doc, `aggs.${column.id}.aggs.timecolumn.aggs.${metric.id}`, {
            bucket_script: {
              buckets_path: {
